...
DEFAULT_OUTPUTS = [
    "Wetland inflow (m3/day)",
    "Wetland Cell 1 storage (m3)",
    "HRT",
]
...
def canonical_output_name(name: str) -> str:
    """Normalize output names for robust matching.

    Handles either observedoutput.txt headers such as:
        Wetland inflow (m3/day)
    or observation CSV headers such as:
        Obs_Wetland inflow (m3/day)_expression
    """
    x = str(name).strip()
    x = re.sub(r"^Obs_", "", x)
    x = re.sub(r"_expression$", "", x)
    x = re.sub(r"\([^)]*\)", "", x)  # ignore units for fallback matching
    x = re.sub(r"[_\s]+", " ", x)
    return x.strip().lower()


def resolve_requested_outputs(requested: List[str], available: set[str]) -> List[str]:
    """Resolve requested names to actual observedoutput.txt headers."""
    ...
    for req in requested:
        ...
        if req in available:
            ...
            creq = canonical_output_name(req)
            matches = [a for a in available_list if canonical_output_name(a) == creq]
            if not matches:
                matches = [
                    a for a in available_list
                    if creq and (creq in canonical_output_name(a) or canonical_output_name(a) in creq)
                ]
            if matches:
                actual = matches[0]

        if actual is None:
            print(f"[WARN] Requested output not found: {req}")
            continue
        if actual not in resolved:
            if actual != req:
                print(f"[INFO] Matched requested output {req!r} -> observed header {actual!r}")
...
def pretty_parameter_label(param: str) -> str:
    aliases = {
        ...
        "WetlandInflow": "Wetland inflow",
        "WetlandInletStage": "Wetland inlet stage",
        "WetlandMidStage": "Wetland mid-stage",
        "WetlandOutletStage": "Wetland outlet stage",
        "WetlandOutflow": "Wetland outflow",
        "WetlandCell1Storage": "Cell 1 storage",
        "WetlandCell2Storage": "Cell 2 storage",
        "WetlandCell3Storage": "Cell 3 storage",
        "WetlandCell4Storage": "Cell 4 storage",
        "WetlandCell5Storage": "Cell 5 storage",
        "WetlandCell6Storage": "Cell 6 storage",
        "HRT": "HRT",
    }
    if param in aliases:
...
def main() -> None:
    ...
    outputs = resolve_requested_outputs(args.outputs, available)
    if not outputs:
        print("[INFO] Available observed-output headers:")
